"""Delete only quant output CSV files.

This script removes CSV files under data/quant_outputs recursively and keeps
all directory structure and .gitkeep files intact.
...
QUANT_OUTPUTS_DIR = ROOT_DIR / "data" / "quant_outputs"
KEEP_FILES = {".gitkeep"}
...
def collect_targets() -> list[Path]:
    if not QUANT_OUTPUTS_DIR.exists():
        return []

    targets = [
        path
        for path in iter_files(QUANT_OUTPUTS_DIR)
        if path.suffix.lower() == ".csv" and path.name not in KEEP_FILES
    ]
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Clean quant output CSV files.")
    parser.add_argument(
        "--force",
        action="store_true",
