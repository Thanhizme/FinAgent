...
class AnalysisAgent:
    ...
    def _validate_section_response(self, section: str, text: str, tickers: list[str], min_chars: int) -> bool:
        ...
        if section == "trend":
            ...
            if any(token in low for token in trend_tokens):
                return True
            # Accept narrative trend text even without explicit trend keyword.
            return not self._looks_truncated(text)
        if section == "anomaly":
            if "outlier" not in low and "anomal" not in low:
                return False
            if self._looks_truncated(text):
                return False
            return True
        if section == "risk":
            if any(token in low for token in ("risk", "volatility", "drawdown", "sharpe")):
                return True
            return not self._looks_truncated(text)
        if section == "comparison":
            if len(tickers) >= 2:
                if any(token in low for token in ("winner", "better", "outperform", "verdict", "relative", "compare", "return", "volatility", "sharpe")):
                    return True
                return not self._looks_truncated(text)
            return True
        return True
...
2) Include all tickers present in context.
3) Mention concrete numbers/dates from context.
4) Do not return bullet-only numeric dumps.
5) Ensure complete sentences and no truncation.
"""

        user_prompt = (
            "Create full analysis for the following structured data context.\n\n"
            f"TICKERS: {tickers}\n"
            f"COMPARISON_TICKERS: {compare_tickers}\n\n"
            f"DATA_CONTEXT_JSON:\n{context_json}\n"
        )
        ...
        if not self._validate_section_response("trend", trend, tickers, min_chars=220):
            return None
        if not self._validate_section_response("anomaly", anomaly, tickers, min_chars=220):
            return None
        if not self._validate_section_response("risk", risk, tickers, min_chars=220):
            return None
        if not self._validate_section_response("comparison", comparison, compare_tickers, min_chars=220):
            return None
...
"""
            ...
            try:
                ...
                for section in missing:
                    ...
                    tickers_for_check = list(data.keys()) if section != "comparison" else compare_tickers
                    section_name_map = {
                        "trend_summary": "trend",
                        "anomaly_report": "anomaly",
                        "risk_commentary": "risk",
...
Provide a risk commentary with classifications and recommendations."""
    ...
    Produce a complete side-by-side markdown comparison.
    Required sections:
...
Provide a comprehensive comparison highlighting relative strengths and weaknesses."""
        ...
        try:
            ...
            retry_prompt = user_prompt + "\n\nRewrite as a complete comparative narrative with a final verdict and no truncation."
            response_retry = self._call_llm(system_prompt, retry_prompt)
    ...
    def generate_full_analysis(
        ...
        fallback_reason = ""
        try:
            report = self._generate_full_analysis_once(data, comparison_tickers)
            if report is None:
        ...
        if report is None:
            sectional: Dict[str, Any] = {
                "trend_summary": self.generate_trend_summary(data),
                "anomaly_report": self.generate_anomaly_report(data),
                "risk_commentary": self.generate_risk_commentary(data),
